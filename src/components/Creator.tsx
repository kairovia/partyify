...
function prepForRender(
	...
	preProcessedImageCtx.imageSmoothingEnabled = true;
	preProcessedImageCtx.imageSmoothingQuality = 'high';
...
function generateCachedFrames(
	...
	const scaledCanvasWidth = imageSizing.canvasWidth * renderScale;
	const scaledCanvasHeight = imageSizing.canvasHeight * renderScale;
...
function renderFrame(
	...
	canvasCtx.setTransform(renderScale, 0, 0, renderScale, 0, 0);
	canvasCtx.clearRect(0, 0, imageSizing.canvasWidth, imageSizing.canvasHeight);
	canvasCtx.transform(
		frameTransformation[0] ?? 1,
		frameTransformation[1] ?? 0,
		frameTransformation[2] ?? 0,
		frameTransformation[3] ?? 1,
		frameTransformation[4] ?? 0,
		frameTransformation[5] ?? 0
	);
	// draw base image
	canvasCtx.globalCompositeOperation = 'source-over';
...
export default function Creator(): JSX.Element {
	...
	useEffect(() => {
		...
		previewCanvas.width = cachedData.crop.right - cachedData.crop.left;
		previewCanvas.height = cachedData.crop.bottom - cachedData.crop.top;
	...
	}, [imageSource, imageMeta, settings, imagePrepCanvas, lastRenderIteration, lastRenderTime]);
	...
	const generateGif = () => {
		...
		for (let frameNumber = 0; frameNumber < frameCount; frameNumber++) {
			...
			gifRenderer.addFrame(outputProcessingCtx, { delay: gifFrameDelay, copy: true });
		}
	...
	return (
		<section className='Creator'>
			...
			<div className='Output' ref={outputElementRef}>
				{outputImageBlobUrl && (
					<>
						<p>Right click or long-press the image below, then save it to use the loop elsewhere.</p>
